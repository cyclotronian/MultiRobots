#ifndef __VMULTIROBOTTASK_H
#define __VMULTIROBOTTASK_H
...
nmMap
host
set blockId to the blockID!!
*/
extern void multiRobotVMTask( void *pvParameters );
/******************************************************************************
...
/* Reg num of regis containing tuple to send */
#define SEND_MSG(x)   (*(const unsigned char*)(x))
/* Reg num of regis containing faceNum to send to */
...
/* Returns the regis num of the regis containing tuple to remove */
...
/* Alternative way of getting name char * for type x */
...
/* This block's ID */
static NodeID blockId = 1;
/* An array of 32 registers (pointers) */
static Register regis[32];
...
static uint64_t currentLocalDate;
static bool hasWork, polling, deterministicSet;
static NodeID neighbors[3];
static tuple_type TYPE_INIT;
static tuple_type TYPE_EDGE;
static tuple_type TYPE_TERMINATE;
static tuple_type TYPE_NEIGHBORCOUNT;
static tuple_type TYPE_NEIGHBOR;
static tuple_type TYPE_VACANT;
static tuple_type TYPE_TAP;
static tuple_type TYPE_SETCOLOR;
static tuple_type TYPE_SETCOLOR2;
static tuple_type TYPE_POSITION;
static tuple_type TYPE_READY;
static tuple_type TYPE_SETPOSITION;
static tuple_type TYPE_MOVETO;
static tuple_type TYPE_READYCOUNT;
static tuple_type TYPE_UNBROADCASTED;
...
static void setConfiguration(char * path, bool d);
static void readProgram();
static int characterCount(const char * in, char character);
...
NodeID getGUID();
static extern_funct_type *extern_functs;
static int *extern_functs_args;
...
static tuple_queue receivedTuples[NUM_PORTS];

/*static Why static again ?*/
static tuple_t tuple_alloc(tuple_type type)
{
...
void tuple_do_handle(tuple_type type, void *tuple, int isNew, Register *regis);
...
void execute_run_action (const unsigned char *pc, Register *regis, int isNew);
void * eval_reg(const unsigned char value, const unsigned char **pc, Register *regis);
void * eval_int (const unsigned char **pc);
void * eval_float (const unsigned char **pc);
void moveTupleToReg (const unsigned char reg_index, tuple_t tuple, Register *regis);
void execute_addtuple (const unsigned char *pc, Register *regis, int isNew);
void execute_send_delay (const unsigned char *pc, Register *regis, int isNew);
void execute_alloc (const unsigned char *pc, Register *regis);
void execute_update (const unsigned char *pc, Register *regis);
void execute_send (const unsigned char *pc, Register *regis, int isNew);
void execute_call1 (const unsigned char *pc, Register *regis);
void execute_run_action0 (tuple_t action_tuple, tuple_type type, int isNew);
void execute_remove (const unsigned char *pc, Register *regis, int isNew);
void execute_mvintfield (const unsigned char *pc, Register *regis);
void execute_mvintreg (const unsigned char *pc, Register *regis);
void execute_mvfloatreg (const unsigned char *pc, Register *regis);
void execute_mvfloatfield (const unsigned char *pc, Register *regis);
void execute_mvfieldreg (const unsigned char *pc, Register *regis);
void execute_mvfieldfield (const unsigned char *pc, Register *regis);
void execute_mvregfield (const unsigned char *pc, Register *regis);
void execute_mvhostfield (const unsigned char *pc, Register *regis);
void execute_mvhostreg (const unsigned char *pc, Register *regis);
void execute_mvregreg (const unsigned char *pc, Register *regis);
void execute_not (const unsigned char *pc, Register *regis);
void execute_boolor (const unsigned char *pc, Register *regis);
void execute_boolequal (const unsigned char *pc, Register *regis);
void execute_boolnotequal (const unsigned char *pc, Register *regis);
void execute_addrequal (const unsigned char *pc, Register *regis);
void execute_addrnotequal (const unsigned char *pc, Register *regis);
void execute_intequal (const unsigned char *pc, Register *regis);
void execute_intnotequal (const unsigned char *pc, Register *regis);
void execute_intgreater (const unsigned char *pc, Register *regis);
void execute_intgreaterequal (const unsigned char *pc, Register *regis);
void execute_intlesser (const unsigned char *pc, Register *regis);
void execute_intlesserequal (const unsigned char *pc, Register *regis);
void execute_intmul (const unsigned char *pc, Register *regis);
void execute_intdiv (const unsigned char *pc, Register *regis);
void execute_intmod (const unsigned char *pc, Register *regis);
void execute_intplus (const unsigned char *pc, Register *regis);
void execute_intminus (const unsigned char *pc, Register *regis);
void execute_floatplus (const unsigned char *pc, Register *regis);
void execute_floatminus (const unsigned char *pc, Register *regis);
void execute_floatmul (const unsigned char *pc, Register *regis);
void execute_floatdiv (const unsigned char *pc, Register *regis);
void execute_floatequal (const unsigned char *pc, Register *regis);
void execute_floatnotequal (const unsigned char *pc, Register *regis);
void execute_floatlesser (const unsigned char *pc, Register *regis);
void execute_floatlesserequal (const unsigned char *pc, Register *regis);
void execute_floatgreater (const unsigned char *pc, Register *regis);
void execute_floatgreaterequal (const unsigned char *pc, Register *regis);
...
void aggregate_recalc(tuple_entry *agg, Register *regis, bool first_run);
...
struct eventsList
{
  Time eventsTime;
  int eventsType;
  struct eventsList *link;
};
#define MAX_EVENTLISTPOOL 100
static struct eventsList eventsListPool[MAX_EVENTLISTPOOL];
static int eventsListPoolSize = 0;
struct eventsList* malloc_eventsList();

static struct eventsList *front=NULL;

struct localEventsList
{
    Time eventsTime;
    int eventsType;
    struct localEventsList *link;
};

static struct localEventsList *localfront=NULL;
...
// void create_localEventsList();
int sizeof_localEventsList();
static int count_localEvents = 0;

static time_t rawtime,starttime;
...
void ControlRobotgo2Position(float , float , float );

enum State {NOTREADY = 0, NOTSTARTED = 1, ENDED = 2, PAUSED =3, RUNNING = 4};
static enum State state;

static meld_int targetPos[3]={0};
static int targetID=0;
static Time startTime, currentDate=0, timeElapsed;
static int haltornot=1;
Time getTime();
void broadCastInfos();
#define FRONT_SIGHT_LENGTH 20
#endif